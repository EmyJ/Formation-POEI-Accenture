 ...
 */
@WebServlet(urlPatterns = { "/nouvelle", "/supprimer" })
public class ServletNouvelleListe extends HttpServlet {
	...
	if (request.getServletPath().equals("/nouvelle")) {
	    creerListeCourses(request);
	} else if (request.getServletPath().equals("/supprimer")) {
	    supprimerArticle(request);
	}
...
/*******************************************Méthode private***********************************************************/ 
    ...
    private void supprimerArticle(HttpServletRequest request) {
	ListeCourseManager listeCourseManager = new ListeCourseManager();
    ...
    private void creerListeCourses(HttpServletRequest request) {
	// Je lis les paramètres
	int idListeCourse = 0;
	List<Integer> listeCodesErreur = new ArrayList<>();
