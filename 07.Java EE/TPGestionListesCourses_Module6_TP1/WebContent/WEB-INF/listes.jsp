				...
				<c:when test="${listesCourse.size()>0}">
					<ul class="list-group col-12">
						<c:forEach var="c" items="${listesCourse}">
							<li
								class="list-group-item d-flex justify-content-between align-items-center">${c.nom}
								<div>
									<a
										href="${pageContext.request.contextPath}/nouvelle?id=${c.id}"
										class="badge" title="Editer la liste"><i
										class="material-icons">create</i></a> <a
										href="${pageContext.request.contextPath}/panier?id=${c.id}"
										class="badge" title="Commencer ses courses"><i
										class="material-icons">shopping_cart</i></a> <a
										href="${pageContext.request.contextPath}/listes?supprimer=${c.id}"
										class="badge text-danger" title="Supprimer"
										onclick="if(window.confirm('Voulez-vous vraiment supprimer la liste ?')){return true;}else{return false;}"><i
